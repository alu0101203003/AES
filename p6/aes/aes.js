...
function generar (){
  ...
  var salida = rinjdael(clave,texto)
  imprimirSalida(datos,salida)
}
...
function get_datos (){
    ...
    var clave_array = clave.match(/.{1,2}/g); 
    var texto_array = texto.match(/.{1,2}/g);
...
function imprimirSalida(datos,salida){
  document.getElementById("s_clave").innerHTML = datos.clave.join("");
  document.getElementById("s_texto").innerHTML = datos.texto.join("");

  var txt = ""
  for (var i = 0; i < salida.subclaves.length; i++){
    var aux = "R"+i+" (Subclave = "+salida.subclaves[i]+") = "+salida.estados[i]+"\n"+"\n"
    txt = txt + aux
  }

  document.getElementById("s_salida").innerHTML = txt;
  document.getElementById("s_cifrado").innerHTML = salida.cifrado;
